...
function updateOptionButtonValue(handles, fieldDictionary, value) {
  ...
  if (isWidget || !fieldDictionary.exists("Kids")) {
    ...
    if (value === null) {
      ...
      var apDictionary = handles.reader
    ...
    var fieldsReferences = writeKidsAndEndObject(
      handles,
      modifiedDict,
      kidsArray
    );
    ...
    for (var i = 0; i < fieldsReferences.length; ++i) {
      ...
      var widgetDictionary = handles.reader
      ...
      var nAppearances = handles.reader
        ...
        .toJSObject();
      const match = _.some(
        Object.keys(nAppearances),
        i => i === value || (value === true && i === "Yes")
      );
      ...
      modifiedFieldDict
        .writeKey("AS")
        .writeNameValue(
          match ? (value === true ? "Yes" : value || "Off") : "Off"
        );
...
function updateTextValue(
  ...
  var modifiedDict = startModifiedDictionary(
    handles,
    fieldDictionary,
    fieldsToRemove
  );
...
function updateFieldWithValue(
  ...
  var localFieldType = fieldDictionary.exists("FT")
...
function writeFilledField(
  ...
  var localFieldNameT = fieldDictionary.exists("T")
...
function fillForm(writer, data) {
  ...
  var acroformDict = catalogDict.exists("AcroForm")
  ...
  if (
    ...
    var acroformObjectId = acroformInCatalog
